...
csv_file = open("Outdoor - TSS.csv", "r", encoding="utf-8")
#リスト形式
data = csv.reader(csv_file, delimiter=",", doublequote=True,  quotechar='"', skipinitialspace=True)
header = next(data)

X = []
y = []
for row in data:
         X.append([float(cell) for cell in row[2:-10]])
...
print(X_feature_names,y_feature_names)
print(X,y)

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25)

svr = SVR(kernel='linear', C=1000.0, epsilon=0.5)
